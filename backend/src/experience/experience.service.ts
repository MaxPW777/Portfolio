...
export class ExperienceService {
  public constructor(
    @InjectModel(Experience.name) private experienceModel: Model<IExperienceDocument>,
    private s3Service: S3Service,
  ) {}

  public async getAllExperiences(): Promise<IExperienceDocument[]> {
    return this.experienceModel.find().sort({ startDate: 1 }).exec();
  }

  public async addExperience(experience: CreateExperienceDto, file: Express.Multer.File): Promise<IExperienceDocument> {
    let imageUrl: string;
    if (file) {
  ...
  public async deleteExperience(id: string): Promise<IExperienceDocument> {
